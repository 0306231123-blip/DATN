...
class AuthController {
  ...
  async login(req, res) {
    try {
      ...
      // Kiểm tra tài khoản bị khóa vĩnh viễn (do Admin khóa)
      ...
      if (nguoiDung.thoi_gian_khoa_tam_thoi && nguoiDung.thoi_gian_khoa_tam_thoi > now) {
          const diffMins = Math.ceil((nguoiDung.thoi_gian_khoa_tam_thoi - now) / 60000);
          return res.status(403).json({
              success: false,
              message: `Tài khoản tạm thời bị khóa do đăng nhập sai quá nhiều. Vui lòng thử lại sau ${diffMins} phút.`
          });
      }
      ...
      if (!isMatch) {
        // Xử lý logic khóa tài khoản tạm thời
        let failedAttempts = nguoiDung.so_lan_dang_nhap_sai || 0;
        let lastFailedTime = nguoiDung.thoi_gian_sai_cuoi;

        // Nếu lần sai cuối cách đây quá 5 phút thì reset bộ đếm
        if (lastFailedTime && (now - lastFailedTime) > (5 * 60 * 1000)) {
            failedAttempts = 1;
        } else {
            failedAttempts += 1;
        }
        ...
        const updateData = {
            so_lan_dang_nhap_sai: failedAttempts,
            thoi_gian_sai_cuoi: now
        };

        if (failedAttempts >= 5) {
            // Khóa tạm thời 15 phút
            updateData.thoi_gian_khoa_tam_thoi = new Date(now.getTime() + 15 * 60 * 1000);
        }
        ...
        if (failedAttempts >= 5) {
            msg = 'Tài khoản của bạn đã bị khóa tạm thời 15 phút do nhập sai mật khẩu 5 lần.';
        } else {
            msg += ` Bạn còn ${5 - failedAttempts} lần thử.`;
        }
      ...
      // Đăng nhập thành công -> Xóa các cờ liên quan đến khóa tạm thời
      if (nguoiDung.so_lan_dang_nhap_sai > 0 || nguoiDung.thoi_gian_khoa_tam_thoi) {
          await nguoiDung.update({
              so_lan_dang_nhap_sai: 0,
              thoi_gian_sai_cuoi: null,
              thoi_gian_khoa_tam_thoi: null
          });
      }
