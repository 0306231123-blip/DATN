            ...
            @foreach($danhSachBanChay as $sp)
                <a href="/user/detail/{{ $sp->ma_san_pham }}" class="bg-white rounded-3xl shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-300 overflow-hidden flex flex-col border border-gray-100 relative">
                    ...
                    <div class="p-4 flex flex-col items-center flex-1">
                        <div class="text-gray-800 font-bold text-sm mb-2 text-center line-clamp-2 h-10">
                            {{ $sp->ten_san_pham }}
                        </div>
                        ...
                        <div class="flex items-center justify-center mt-1 mb-2">
                            <div class="flex text-yellow-400">
                                ...
                                @endphp
                                @for($i = 1; $i <= 5; $i++)
                                    <svg class="w-4 h-4 {{ $i <= $diemTron ? 'text-yellow-400' : 'text-gray-300' }}" viewBox="0 0 24 24" fill="currentColor"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg>
                                @endfor
                            </div>
                            <span class="text-xs text-gray-400 ml-1">({{ $tongLuot > 0 ? $tongLuot : 'Chưa có' }})</span>
                        </div>
