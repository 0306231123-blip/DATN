...
<section class="user-section" style="padding-top: 1.5rem;">
    <div class="user-container">
        ...
        <div class="detail-grid">
            ...
            <div class="detail-info">
                <div class="detail-info__header" style="padding: 0 0 1rem 0; margin-bottom: 1rem; text-align: left; border-bottom: none;">
                    <h1 class="detail-info__title" style="font-weight: 500; font-size: 1.875rem; color: #000; margin-bottom: 0.5rem; text-align: left;">{{ $sanPham->ten_san_pham }}</h1>
        ...
        <div class="detail-reviews" style="margin-top: 2rem;">
            ...
            <div style="display: flex; flex-direction: column; gap: 1rem; margin-bottom: 2.5rem; font-size: 1rem; color: var(--user-gray-800);">
                ...
                @php
                    ...
                    $loaiDa = $loaiDaMapping[$sanPham->loai_da_phu_hop] ?? $sanPham->loai_da_phu_hop;
                @endphp
                <div style="display: flex;"><span style="color: var(--user-gray-500); width: 140px; font-weight: 600;">Dành cho da:</span><span style="font-weight: 500;">{{ $loaiDa }}</span></div>
            </div>
            ...
            @if(!empty($sanPham->huong_dan_su_dung))
            <h3 class="detail-reviews__title" style="margin-top: 2rem;">Hướng dẫn sử dụng</h3>
            <div style="font-size: 1rem; color: var(--user-gray-700); line-height: 1.8; white-space: pre-line;">{{ $sanPham->huong_dan_su_dung }}</div>
            @endif
        </div>
        ...
        <div class="detail-reviews">
            ...
            <!-- Review Filter UI -->
            <div id="review-filter-container" class="hidden flex-wrap gap-2 mb-6 items-center bg-[#fffbf8] p-5 border border-orange-100 rounded-sm">
            ...
            <!-- Pagination UI -->
            <div id="review-pagination" class="hidden flex justify-end gap-1 mt-8 items-center">
...
<script>
    ...
    function renderReviewFilters() {
        ...
        const counts = { 0: allReviews.length, 5: 0, 4: 0, 3: 0, 2: 0, 1: 0 };
        allReviews.forEach(r => {
            if (counts[r.diem_so] !== undefined) counts[r.diem_so]++;
        });
        ...
        let html = '';
        filters.forEach(f => {
            const isActive = currentFilterStar === f.value;
            const activeClasses = isActive 
                ? 'bg-[#ee4d2d] text-white border-[#ee4d2d]' 
                : 'bg-white text-gray-700 border-gray-200 hover:border-[#ee4d2d] hover:text-[#ee4d2d]';
            
            html += `
                <button onclick="setReviewFilter(${f.value})" class="px-5 py-2 border rounded-sm text-sm transition-colors ${activeClasses}">
    ...
    window.setReviewFilter = function(star) {
    ...
    window.changeReviewPage = function(page) {
        ...
        renderReviews();
        // Scroll to top of reviews slightly
        const reviewSection = document.getElementById('review-filter-container');
        if (reviewSection) {
            const y = reviewSection.getBoundingClientRect().top + window.scrollY - 100;
    ...
    function renderReviews() {
        ...
        const filteredReviews = currentFilterStar === 0 
            ? allReviews 
            : allReviews.filter(r => r.diem_so === currentFilterStar);

        if (filteredReviews.length === 0) {
            container.innerHTML = '<p class="text-center text-gray-500 italic py-8">Chưa có đánh giá nào cho bộ lọc này.</p>';
            paginationContainer.classList.add('hidden');
            return;
        }
        ...
        if (totalPages > 1) {
            ...
            if (reviewCurrentPage > 1) {
                pagHtml += `<button onclick="changeReviewPage(${reviewCurrentPage - 1})" class="w-8 h-8 flex items-center justify-center rounded-sm text-gray-500 hover:text-[#ee4d2d] bg-white border border-gray-200"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg></button>`;
            }

            for (let i = 1; i <= totalPages; i++) {
                if (i === reviewCurrentPage) {
                    pagHtml += `<button class="w-8 h-8 flex items-center justify-center rounded-sm bg-[#ee4d2d] text-white font-medium border border-[#ee4d2d]">${i}</button>`;
                } else {
                    pagHtml += `<button onclick="changeReviewPage(${i})" class="w-8 h-8 flex items-center justify-center rounded-sm text-gray-500 hover:text-[#ee4d2d] bg-white border border-gray-200">${i}</button>`;
                }
            }
